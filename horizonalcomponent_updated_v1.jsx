...
// Modified to accept props from parent component
const HorizontalFlowVisualizer = ({ 
  ...
  // UI state - now accepting props but also maintaining local state
  const [localSelectedComponent, setLocalSelectedComponent] = useState(null);
  ...
  useEffect(() => {
    ...
    if (showDetails !== undefined) {
      setLocalShowDetails(showDetails);
    }
  }, [selectedComponent, showDetails]);
  ...
  const handleSelectComponent = (node) => {
    setLocalSelectedComponent(node);
            ...
            <div className="space-y-6">
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Spans</div>
                    <div className="text-sm font-medium">{component.spanIds ? component.spanIds.length : 0}</div>
                  </div>
                  <div>
                    <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Duration</div>
                    <div className="text-sm font-medium">{formatDuration(component.duration)}</div>
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {outgoingCalls.map((call, index) => (
                      ...
                      >
                        <div>
                          <div className="font-medium text-gray-900">{call.target.name}</div>
                          <div className="text-xs text-gray-500 mt-1">
                            {call.target.spanIds ? `${call.target.spanIds.length} spans` : '0 spans'}
                          </div>
                        </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {incomingCalls.map((call, index) => (
                      ...
                      >
                        <div>
                          <div className="font-medium text-gray-900">{call.source.name}</div>
                          <div className="text-xs text-gray-500 mt-1">
                            {call.source.spanIds ? `${call.source.spanIds.length} spans` : '0 spans'}
                          </div>
                        </div>
                        <div className="flex items-center">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                ...
                endpoints.map((endpoint, index) => (
                  ...
                  >
                    ...
                    <div className="mt-2 flex items-center text-sm text-gray-500">
                      <span className="mr-2">Duration:</span> {formatDuration(endpoint.duration)}
                    </div>
                  </div>
                ))
              ) : (
  ...
  const calculateNodePositions = (nodes, links) => {
    ...
    links.forEach(link1 => {
      links.forEach(link2 => {
        if (link1.source === link2.target && link1.target === link2.source) {
          const pair = [link1.source, link1.target].sort().join('-');
          if (!bidirectionalPairs.includes(pair)) {
    ...
    return { positions, bidirectionalPairs };
  };
  ...
  const FlowDiagram = ({ nodes, links, callCounts }) => {
    ...
    // Calculate positions for all nodes and find bidirectional links
    const { positions, bidirectionalPairs } = calculateNodePositions(nodes, links);
    ...
    return (
      <svg width="100%" height="100%" className="overflow-visible">
        ...
        {links && links.map((link, index) => {
          ...
          const isPairKey = [link.source, link.target].sort().join('-');
          const isBidirectional = bidirectionalPairs.includes(isPairKey);
          
          // For bidirectional links, curve one direction slightly
          const isCurved = isBidirectional && link.source > link.target;
          
          // Calculate path with orthogonal lines (90-degree angles)
          let path;
          if (isCurved) {
            // Create a curved path for one direction of bidirectional
            const midX = (sourcePos.x + targetPos.x + nodeWidth) / 2;
            const controlY = (sourcePos.y + targetPos.y) / 2 - 30;
            path = `M ${sourcePos.x + nodeWidth} ${sourcePos.y + nodeHeight/2}
                   Q ${midX} ${controlY}, ${targetPos.x} ${targetPos.y + nodeHeight/2}`;
          } else {
